...
CREATE TABLE provider_configuration (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE (id, workspace_id),
  UNIQUE (workspace_id, provider_key, capability),
  FOREIGN KEY (workspace_id) REFERENCES workspace(id)
);
...
CREATE INDEX workflow_run_project_created_idx ON workflow_run (project_id, created_at DESC);
...
CREATE TABLE generation_job (
  ...
  input_snapshot jsonb NOT NULL,
  dispatch_seq integer NOT NULL DEFAULT 0 CHECK (dispatch_seq >= 0),
  row_version integer NOT NULL DEFAULT 1 CHECK (row_version > 0),
  ...
  progress_weight integer NOT NULL DEFAULT 1 CHECK (progress_weight > 0),
  ...
  completed_at timestamptz,
  UNIQUE (id, workspace_id),
  UNIQUE (id, workflow_run_id, workspace_id),
  UNIQUE (id, project_id, workspace_id),
  ...
  CHECK (state IN ('RUNNING','WAITING_EXTERNAL') OR (lease_owner IS NULL AND lease_until IS NULL))
);
...
CREATE TABLE job_attempt (
  ...
  provider_client_request_key text NOT NULL,
  ...
  UNIQUE (provider_client_request_key),
  UNIQUE (id, workspace_id),
  UNIQUE (id, generation_job_id, workspace_id),
  UNIQUE (id, provider_configuration_id, provider_request_id, workspace_id),
...
CREATE UNIQUE INDEX job_attempt_provider_request_idx
  ON job_attempt (provider_configuration_id, provider_request_id)
...
CREATE TABLE dispatch_outbox (
  ...
  workspace_id uuid NOT NULL,
  job_id uuid NOT NULL,
  dispatch_seq integer NOT NULL CHECK (dispatch_seq > 0),
...
CREATE TABLE idempotency_record (
  ...
  UNIQUE (workspace_id, actor_id, http_method, route_key, idempotency_key),
  FOREIGN KEY (workspace_id) REFERENCES workspace(id),
...
CREATE TABLE provider_event (
  ...
  source text NOT NULL CHECK (source IN ('CALLBACK','POLL')),
...
CREATE TABLE cost_ledger (
  ...
  basis text NOT NULL CHECK (basis IN ('PROVIDER_REPORTED','LOCALLY_CALCULATED')),
  unit_type text,
  unit_quantity numeric(20,8),
  unit_price_snapshot numeric(20,8),
  provider text NOT NULL,
  model text NOT NULL,
  occurred_at timestamptz NOT NULL DEFAULT now(),
  FOREIGN KEY (project_id, workspace_id) REFERENCES project(id, workspace_id),
  FOREIGN KEY (generation_job_id, project_id, workspace_id)
...
BEGIN
  SELECT state, dispatch_seq, workspace_id
    INTO current_state, current_seq, current_workspace
  FROM generation_job
  WHERE id = NEW.job_id
  FOR KEY SHARE;
...
CREATE TRIGGER dispatch_outbox_matches_job
...
BEGIN
  IF NEW.job_id IS DISTINCT FROM OLD.job_id
    OR NEW.dispatch_seq IS DISTINCT FROM OLD.dispatch_seq
