...
CREATE TABLE idempotency_record (
  ...
  CHECK (request_hash ~ '^[0-9a-f]{64}$'), CHECK ((response_status IS NULL) = (response_body IS NULL))
);
...
CREATE TABLE provider_event (
  ...
  FOREIGN KEY (provider_configuration_id, workspace_id) REFERENCES provider_configuration(id, workspace_id),
  FOREIGN KEY (job_attempt_id, workspace_id) REFERENCES job_attempt(id, workspace_id)
);
...
BEGIN
  ...
  IF current_state <> 'QUEUED' OR current_seq <> NEW.dispatch_seq OR current_workspace <> NEW.workspace_id THEN
