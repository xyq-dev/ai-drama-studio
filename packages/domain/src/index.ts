...
export const GENERATION_JOB_STATES = [
  "PENDING", "QUEUED", "RUNNING", "WAITING_EXTERNAL", "SUCCEEDED", "FAILED", "CANCELED",
] as const;
export type GenerationJobState = (typeof GENERATION_JOB_STATES)[number];

export const WORKFLOW_RUN_STATES = [
  "PENDING", "RUNNING", "SUCCEEDED", "PARTIAL_FAILED", "FAILED", "CANCELED",
] as const;
...
export function deriveWorkflowRunState(jobs: readonly WorkflowJobSummary[]): WorkflowRunState {
  if (jobs.length === 0 || jobs.every(({ state }) => state === "PENDING")) return "PENDING";
  if (jobs.every(({ state }) => state === "CANCELED")) return "CANCELED";
  if (jobs.some(({ state, isCritical }) => isCritical && (state === "FAILED" || state === "CANCELED"))) return "FAILED";
  if (jobs.some(({ state }) => !["SUCCEEDED", "FAILED", "CANCELED"].includes(state))) return "RUNNING";
  if (jobs.some(({ state }) => state === "FAILED" || state === "CANCELED")) return "PARTIAL_FAILED";
  return "SUCCEEDED";
}
