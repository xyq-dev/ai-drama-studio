...
前缀为 `/api/v1`，JSON 使用 UTC ISO-8601 时间、ULID/UUID 字符串和显式分页 cursor。所有写入要求登录；V1 使用单工作区上下文，但服务端仍从认证主体确定 `workspaceId`，不信任客户端传入的工作区。创建异步工作返回 `202 { workflowRun, jobs[] }`。可重试 POST 要求 `Idempotency-Key`；同 key + 相同方法/路径/规范化请求在 24 小时内返回同一结果，冲突返回 `IDEMPOTENCY_KEY_REUSED`。编辑带 `If-Match: aggregate-etag`，冲突返回 `REVISION_CONFLICT`。
...
| `POST /character-revisions/:id/reference-images/generate` | 生成参考图工作流 | Owner；key；`CHARACTER_REVIEW_REQUIRED` |
...
| Method / Path | 用途、输入、输出 | 权限 / 幂等 / 特有错误 |
| --- | --- | --- |
| `POST /assets/uploads` | 创建受限上传会话 `{kind,mime,size}`；返回 URL/Asset draft | Owner；key；`UNSUPPORTED_MEDIA`,`UPLOAD_LIMIT` |
| `POST /assets/:assetId/complete` | 校验上传并使资产可用 | Owner；key；`OBJECT_HASH_MISMATCH` |
...
| `POST /generation-jobs/:jobId/retry` | 新工作或安全重排；返回 accepted | Owner；key；`JOB_NOT_RETRYABLE`,`RETRY_LIMIT` |
...
| `POST /provider-callbacks/:providerKey` | Provider 回调事件；返回 204 | Provider signature；按 event id 幂等；`INVALID_SIGNATURE`,`UNKNOWN_PROVIDER_REQUEST` |
...
`GET /events` 是认证 SSE，支持 `Last-Event-ID`，事件为已提交数据库事件的 at-least-once 投递。客户端须按 `eventId` 去重，断线后重连；若保留窗口已过，重新 GET 资源并轮询。所有事件 `{eventId, occurredAt, traceId, data}`：
...
`approve` 不是 `review` 的第二条路由；最终实现只保留上述 `review` 端点。审核记录固定包含 `subjectType, subjectId, decision, reviewerId, reviewedAt, note, contentHash/revisionId`。任何受审核内容的新当前版本、或其依赖内容变更，都会使相应批准无效；未批准剧本不得生成角色/场景，未批准角色和 ShotRevision 不得生成视频，未批准最终合成 Asset 不得创建 export。

列表端点使用稳定 cursor，默认 `createdAt DESC`，允许白名单 `sort`，并对 `status, assetType, provider, episodeId, shotId, workflowRunId, createdAt` 使用显式过滤；不得接受任意字段/SQL 排序。SSE 的 `Last-Event-ID` 是已提交事件 ID；服务端可重放保留窗口，窗口外返回 `EVENT_CURSOR_EXPIRED` 并由客户端按资源 GET/轮询恢复。所有异步创建、取消、retry、上传完成和当前版本切换端点均为幂等写入；普通元数据 PATCH 以 aggregate ETag 防止覆盖。
