...
同步 Provider 的 `submit` 可返回终态输出；异步 Provider 返回 request id 与 `WAITING_EXTERNAL`，必须实现 `query`，可取消时实现 `cancel`，回调时由 API 使用每个配置的 webhook secret 验签、限流和去重。每次 callback 或 poll 都创建 ProviderEvent，使用 `(provider_configuration_id, provider_request_id, normalized_event_key)` 唯一约束：Provider 给出稳定 event id 时直接使用；否则 Adapter 由 providerRequestId、远程状态版本/更新时间和规范化响应哈希确定性生成，poll 也必须生成该 key，绝不能使用本地随机数。能力发现返回可用模型、输入/输出限制及参数 schema，Adapter 在 submit 前验证 schema。Mock Provider 与相同接口运行，支持同步、异步、重复回调、可重试/不可重试错误和可控 usage；它不声称真实成本或媒体质量。业务模块只依赖 contracts 包，不能直接导入厂商 SDK。
